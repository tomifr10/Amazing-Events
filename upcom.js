...
async function dataAPI() {
    ...
    let dataUpComEvent = dataEventos.events.filter(event => event.date > dataFecha);
    
    let categories = dataUpComEvent.map(evento => evento.category);
    ...
    function creadorTarjPast(filtro){
        ...
        // dataUpComEvent.map((evento,id) =>evento.id = id);
    };
    ...
    function mostarFiltro() {
        ...
        if(filtroCat.length > 0 && textoBusq != "") {
            filtroCat.map(tarjCheck => {
                filtroDef.push(...dataUpComEvent.filter(tarj=> (tarj.category == tarjCheck) && tarj.name.toLowerCase().includes(textoBusq.toLowerCase().trim())))
            });
            ...
            filtroDef.push(...dataUpComEvent.filter(title => 
            ...
            filtroCat.map(tarjCheck => {
                filtroDef.push(...dataUpComEvent.filter(tarj => tarj.category == tarjCheck))
            });
            if(filtroDef.length == 0) {
                document.querySelector(".tarjetas").innerHTML =  `<p class="noFound">No results found.</p>`;
            };
        } else filtroDef.push(...dataUpComEvent);
